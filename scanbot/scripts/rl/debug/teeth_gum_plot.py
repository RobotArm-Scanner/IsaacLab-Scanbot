...
def update_teeth_gum_plot(
    ...
    if not enabled:
        manager.close_plot(env, "teeth_gum_plot")
        return
    manager.ensure_control_window(env)
    if not manager.is_enabled("teeth_gum_plot"):
        manager.close_plot(env, "teeth_gum_plot")
        return
    ...
    if fig is None or not plt.fignum_exists(fig.number):
        ...
        for row, env_id in enumerate(env_ids):
            ...
            env_hist: dict[str, object] = {
                ...
                "y": {},
                "teeth_gum_all": deque(maxlen=max_points),
            }
