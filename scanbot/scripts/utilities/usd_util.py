...
@clone
def spawn_rigid_object_from_usd(
    prim_path: str,
    cfg: UsdFileCfg,
...
@clone
def spawn_usd_with_mesh_collision(
    prim_path: str,
    cfg: UsdFileCfg,
    translation: tuple[float, float, float] | None = None,
    orientation: tuple[float, float, float, float] | None = None,
    **kwargs,
    ...
    """Spawn a USD, apply rigid body on the asset root, and add collision to mesh prims."""
    usd_path = str(cfg.usd_path)
    if "://" not in usd_path and not os.path.isfile(usd_path):
        raise FileNotFoundError(f"USD file not found at path: '{usd_path}'.")
    ...
    if not ref_prim.HasAPI(UsdPhysics.RigidBodyAPI):
        UsdPhysics.RigidBodyAPI.Apply(ref_prim)
    if cfg.rigid_props is not None:
        schemas_utils.define_rigid_body_properties(ref_prim_path, cfg.rigid_props)
